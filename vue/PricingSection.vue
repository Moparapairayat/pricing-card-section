...
const visibleCards = reactive({ 0: false, 1: false, 2: false });
...
// ── Computed ───────────────────────────────────────────────────────────────
const currentCurrency = computed(() => currencyData[currency.value]);

// Animated counter helper (returns formatted string via watcher)
const animatedValues = reactive([0, 0, 0]);
let animFrames = [null, null, null];

function animateTo(index, target) {
  const isDecimal = currentCurrency.value.isDecimal;
  ...
  const step     = (ts) => {
    ...
    animatedValues[index] = ease(progress) * (target - start) + start;
    if (progress < 1) {
...
// ── Watch: re-animate on billing / currency change ─────────────────────────
watch([isYearly, currency], () => {
  [0, 1, 2].forEach((i) => {
    if (visibleCards[i]) {
      const plan = currentCurrency.value.plans[i];
      animateTo(i, isYearly.value ? plan.yearly : plan.monthly);
    }
  });
  if (isYearly.value) {
    pulseActive.value = true;
    setTimeout(() => (pulseActive.value = false), 800);
  }
});
...
onMounted(() => {
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          const plan = currentCurrency.value.plans[idx];
          animateTo(idx, isYearly.value ? plan.yearly : plan.monthly);
  ...
  [card0Ref, card1Ref, card2Ref].forEach((r) => {
  ...
  onUnmounted(() => {
    ...
    observer.disconnect();
    animFrames.forEach((f) => { if (f) cancelAnimationFrame(f); });
  });
});
  ...
  <div class="pricing-page-wrapper">
    ...
    <section class="pricing-section">
      ...
      <div class="pricing-toggle-container">
        <span :class="['toggle-label', { active: !isYearly }]" @click="isYearly = false">Monthly</span>
        <button
          :class="['billing-toggle-btn', { yearly: isYearly }]"
          @click="isYearly = !isYearly"
          aria-label="Toggle billing cycle"
        >
          <span class="toggle-slider"></span>
        </button>
        <span :class="['toggle-label', { active: isYearly }]" @click="isYearly = true">
          Yearly <span class="discount-badge">Save 20%</span>
        </span>

        <!-- Currency Dropdown -->
        <div class="currency-selector-wrapper" ref="dropdownRef">
          <div :class="['custom-dropdown', { open: dropdownOpen }]">
            <button
            ...
            <ul class="dropdown-menu" role="listbox">
              <li
                v-for="key in Object.keys(currencyData)"
                :key="key"
                :class="['dropdown-item', { active: currency === key }]"
                role="option"
      ...
      <div class="pricing-grid">
        ...
        <div
          ref="card0Ref"
          data-index="0"
          :class="['card-wrapper', 'card-wrapper-essential', { visible: visibleCards[0] }]"
          ...
          <div class="card card-essential">
            <div class="card-section">
              ...
              <div class="price-hero">
                ...
                <div class="current-price-wrapper">
                  ...
                  <span class="current-price">{{ visibleCards[0] ? formatVal(0) : '0' }}</span>
                ...
                <div :class="['savings-tracker-banner', { active: isYearly, pulse: pulseActive }]">
            ...
            <div class="card-section">
              ...
              <div class="trust-footer">
                <div class="stars">
                  <svg v-for="i in 5" :key="i" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div
          ref="card1Ref"
          data-index="1"
          :class="['card-wrapper', 'card-wrapper-elite', { visible: visibleCards[1] }]"
          style="transition-delay:0.15s"
        >
          <div class="card card-elite" @mousemove="handleSpotlight">
            ...
            <div class="card-section">
              ...
              <div class="price-hero">
                ...
                <div class="current-price-wrapper">
                  <span class="currency">{{ currentCurrency.symbol }}</span>
                  <span class="current-price">{{ visibleCards[1] ? formatVal(1) : '0' }}</span>
                  <span class="price-suffix">{{ isYearly ? '/year' : '/mo' }}</span>
                </div>
                <div class="monthly-equivalent">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div
          ref="card2Ref"
          data-index="2"
          :class="['card-wrapper', 'card-wrapper-ultimate', { visible: visibleCards[2] }]"
          style="transition-delay:0.3s"
        >
          <!-- Electric Border Layers -->
          <div class="electric-inner-container">
            <div class="electric-border-outer">
              <div class="main-card-electric"></div>
            </div>
          ...
          <div class="card card-ultimate" @mousemove="handleSpotlight">
            ...
            <div class="card-section">
              ...
              <div class="price-hero">
                ...
                <div class="current-price-wrapper">
                  <span class="currency">{{ currentCurrency.symbol }}</span>
                  <span class="current-price">{{ visibleCards[2] ? formatVal(2) : '0' }}</span>
                  <span class="price-suffix">{{ isYearly ? '/year' : '/mo' }}</span>
                </div>
                <div class="monthly-equivalent">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="cta-container">
                <div class="btn-ultimate-wrapper">
...
/*
  Copy react/PricingSection.css alongside this file as PricingSection.css
  OR paste the full CSS content directly inside this <style> block.
*/
</style>
